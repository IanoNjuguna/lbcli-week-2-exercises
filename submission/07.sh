#!/usr/bin/bash
# Create a raw transaction with an amount of 20,000,000 satoshis to this address: 2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP 
# Use the UTXOs from the transaction below
raw_tx=""

RECIPIENT_ADDRESS="2MvLcssW49n9atmksjwg2ZCMsEMsoj3pzUP"
TX_ID=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.txid')
VOUT_INPUT=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vin[0].vout')

# Get total value of the input UTXO to calculate change
INPUT_VAL=$(bitcoin-cli -regtest decoderawtransaction "$raw_tx" | jq -r '.vout[0].value')

# Build with clean JSON
INPUTS=$(jq -n --arg txid "$TX_ID" --argjson vout "$VOUT_INPUT" '[{txid: $txid, vout: $vout}]')
OUTPUTS=$(jq -n --arg addr "$RECIPIENT_ADDRESS" --argjson amt 0.2 '{($addr): $amt}')

# Create unsigned raw tx
RAW_HEX=$(bitcoin-cli -regtest createrawtransaction "$INPUTS" "$OUTPUTS")

# Sign tx
SIGNED_DATA=$(bitcoin-cli -regtest signrawtransactionwithwallet "$RAW_HEX")

# Broadcast to the network
SIGNED_HEX=$(bitcoin-cli -regtest signrawtransactionwithwallet "$RAW_HEX" | jq -r '.hex')
bitcoin-cli -regtest sendrawtransaction "$SIGNED_HEX"
